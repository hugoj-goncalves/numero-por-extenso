import nomes from './nomesExtenso/nomes';
import dezenas from './nomesExtenso/dezenas';
...
function numberInFull(number) {
    ...
    if (numbers.length && numbers[0].number > 0) {
        const e = numbers.length > 1 && (numbers[0].number < 100 || numbers[0].number % 100 == 0);
        output.push(e ? 'e ' : '' + numbers[0].name)
    }
